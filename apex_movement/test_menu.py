...
check("the menu holds one line per movement", [group.display_name for group in menu.MENU] == [
    "Auto sprint (On)", "Slides", "Axle slide (Off)", "Dash", "Air crouch (On)", "Air strafe (On)", "Heavier fall (On)",
    "Wall climb (On)",
])
check("each movement's line opens its switch and settings", menu.auto_sprint.children == [
    settings.auto_sprint, settings.walk_speed, settings.sprint_speed,
] and menu.heavier_fall.children == [settings.heavier_fall, settings.fall_weight, settings.jump_height_bonus])
...
check("air crouch holds its landing minimum", settings.landing_slide_min_speed in menu.air_crouch.children)
...
check("switching a movement off shows on its line", menu.air_strafe.display_name == "Air strafe (Off)")
settings.air_strafe.value = True
check("switching it back on shows too", menu.air_strafe.display_name == "Air strafe (On)")
