...
check("a dropped request is reported, once", sum("request dropped" in line for line in state["errors"]) == 1)

air_keys.crouch_event(pad, "IE_Pressed", 8100 * MS, True)
air_keys.crouch_event(pad, "IE_Released", 8150 * MS, True)
air_keys.reset()
check("reset drops the requests still waiting, so none is played later", air_keys.due_requests(8200 * MS) == [])

# A menu opened with the key down: the release never reaches the mod. The next press proves the last one is over.
air_keys.crouch_event(pad, "IE_Pressed", 8500 * MS, True)
air_keys.jump_event("IE_Pressed", 9000 * MS)
air_keys.crouch_event(pad, "IE_Pressed", 9030 * MS, True)
check("a slam press after an unseen release does not count as held", not air_keys.is_held())
air_keys.crouch_event(pad, "IE_Released", 9060 * MS, True)
air_keys.reset()
air_keys.crouch_event(pad, "IE_Pressed", 9100 * MS, False)
air_keys.jump_event("IE_Pressed", 9200 * MS)
air_keys.crouch_event(pad, "IE_Pressed", 9230 * MS, True)
check("nor after an unseen release of a ground press", not air_keys.is_held())
air_keys.crouch_event(pad, "IE_Released", 9260 * MS, True)
air_keys.reset()
air_keys.crouch_event(pad, "IE_Pressed", 9300 * MS, True)
air_keys.crouch_event(pad, "IE_Pressed", 9400 * MS, False)
check("a ground press after an unseen release has its release passed too",
      not air_keys.crouch_event(pad, "IE_Released", 9450 * MS, False))
air_keys.reset()

air_keys.crouch_event(pad, "IE_Pressed", 10000 * MS, True)
air_keys.jump_event("IE_Pressed", 10500 * MS)
air_keys.crouch_event("LeftControl", "IE_Pressed", 10400 * MS, False)
air_keys.reset()
check("reset forgets held presses, ground ones too", not air_keys.is_held())
check("reset forgets the last jump", air_keys.crouch_event(pad, "IE_Pressed", 10520 * MS, True))
air_keys.reset()
check("a release after reset passes to the game", not air_keys.crouch_event(pad, "IE_Released", 10600 * MS, True))
