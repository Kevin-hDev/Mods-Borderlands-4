...
from . import control_console_handoff, control_view
...
def cancel():
    if _active is not None:
        if _active.closed:
            _active.recover(time.perf_counter_ns())
        else:
...
class Session:
    ...
    def same_context(self, pc):
        ...
        return (pc is not None and pc == get_pc(possibly_loading=True)
                and (self.frontend or character is not None)
                and getattr(pc, "OakCharacter", None) == character)
