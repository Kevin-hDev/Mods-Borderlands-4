...
from .control_window_hooks import HOOK, IDENTIFIER, note
...
class Cleanup:
    ...
    def advance(self, now, explicit=False):
        ...
        for name in STEPS:
            if name not in self.done and self.run(name, getattr(self, name)):
                self.done.add(name)
    ...
    def menu(self):
        ...
        if self.reason == "button" and self.session.handoff is not None and self.context() is not None:
            self.session.handoff.restore()

    def hook(self):
        if self.session.hooked:
            unrealsdk.hooks.remove_hook(HOOK, unrealsdk.hooks.Type.POST, IDENTIFIER)
            if unrealsdk.hooks.has_hook(HOOK, unrealsdk.hooks.Type.POST, IDENTIFIER):
                raise RuntimeError("Window callback still registered")
