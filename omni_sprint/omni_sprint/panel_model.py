"""Validated settings transactions; Omni Sprint's SDK options are the authority."""
...
class Model:
    def __init__(self, mod):
        ...
        # While another higher-priority camera mod is on, Omni Sprint's settings would change nothing.
        # The page then shows where to set the camera instead (Kevin, 2026-09-25).
        self.camera_elsewhere = camera.elected_elsewhere()
        hidden = {option.identifier for option in menu.camera.children} if self.camera_elsewhere else set()
        self.options = {option.identifier: option for group in self.groups for option in group.children
                        if option.identifier not in hidden}
        self._undo = ()
    ...
    def normalize(option, value):
        if isinstance(option, KeybindOption):
            return settings.normalize_keyboard_key(value)
